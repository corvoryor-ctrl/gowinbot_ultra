# strategies/macd_ema_psar_1m.py
# Strategy: MACD(4,9,4) + EMA12 + Parabolic SAR for 1m
# - รองรับ candles เป็น pandas.DataFrame หรือ list[dict]
...
class MacdEmaPsar1mStrategy:
    name = "MACD+EMA12+PSAR (1m)"
    tf   = 1  # นาที
    ...
    def _apply_sensitivity(self):
        ...
        # EMA length: 20 → 8 (สูง = ไว)
        self.ema_len      = int(round(_lerp(20, 8, t)))

        # MACD 4/9/4 baseline — ต่ำ = ยาว / สูง = สั้น
        self.macd_fast    = int(round(_lerp(6, 3, t)))
        self.macd_slow    = int(round(_lerp(14, 7, t)))
        self.macd_signal  = int(round(_lerp(6, 3, t)))

        # PSAR (สูง = เร็ว/ไล่ราคาไว)
        self.psar_af      = _lerp(0.01, 0.04, t)
        self.psar_af_step = _lerp(0.01, 0.04, t)
        self.psar_af_max  = _lerp(0.12, 0.20, t)

        # Flip / slope windows
        self.lookback_flip = int(round(_lerp(4, 1, t)))
        self.slope_window  = int(round(_lerp(5, 1, t)))

        # Histogram gate & min confidence
        self.min_hist_abs  = _lerp(0.0015, 0.0, t)
        self.min_conf      = round(_lerp(0.75, 0.50, t), 2)
        ...
        try:
            ...
            for r in candles:
                if not isinstance(r, dict):
                    # บาง engine อาจให้ namedtuple / object — ใช้ getattr สำรอง
    ...
    def evaluate(self, asset: str, candles) -> Dict[str, Any]:
        ...
        # เช็คแบบ explicit (ไม่ใช้ truthiness ของ DataFrame)
